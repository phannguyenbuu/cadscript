...
namespace AcadScript
{
    public class IBeamCLS
    {
            ...
            bool has_text = true, bool has_pointed = true, double len = 500)
        {
            ...
            if (has_text)
            {
                ...
                ACD.DB.DrawCircle(pt + p4, 125, GP.DEF_LAYER_TEXT);
    ...
    {
        static double C = "<C/>".ToNumber(50);
        //static double E = "<E/>".ToNumber(50);
        static int NST = (int)"<NST/>".ToNumber(2); //SỐ LƯỢNG THÉP CHỦ TRÊN
        static int NSM = (int)"<NSM/>".ToNumber(2); //SỐ LƯỢNG THÉP CHỦ DƯỚI
        static int NSB = (int)"<NSB/>".ToNumber(2); //SỐ LƯỢNG THÉP CHỦ DƯỚI
        static double S1 = "<S1/>".ToNumber(4); 
        static double S2 = "<S2/>".ToNumber(8);
        static double CL1 = "<CL1/>".ToNumber(200); //THÉP ĐAI GỐI
        static double CL2 = "<CL2/>".ToNumber(200); //THÉP ĐAI BỤNG
        static double CLP1 = "<CLP1/>".ToNumber(100); //KHOẢNG CÁCH THÉP ĐAI GỐI
        static double CLP2 = "<CLP2/>".ToNumber(200); //KHOẢNG CÁCH THÉP ĐAI BỤNG
        static double STC = "<STC/>".ToNumber(6); //THÉP ĐAI
        static double STM = "<STM/>".ToNumber(16);
        static double STS = "<STS/>".ToNumber(12);
        static double F = "<F/>".ToNumber(500);

        static double BW = "<BW/>".ToNumber(200);
