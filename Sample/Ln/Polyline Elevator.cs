...
namespace AcadScript
{
    ...
    public class Mesh3DCollection //: PosCollection
    {
        ...
        //void _revised_for_x5()
        //{
        //    foreach (var mesh in this.Elements)
        ...
        public Mesh3DCollection(string[] str, bool _is_x5 = false)
        {
            //Nếu xưởng 5 thì tăng Y lên để bù vào 2 trục cột 4500, nếu không bỏ qua
            Is_X5 = _is_x5;
        ...
        public PosCollection GetFlatView(Mesh3D mesh, pPos axis, pPos __ct)
        {
            ...
            for (int f = 0; f < mesh.FaceIndexes.Count; f++)
            {
                ...
                if (verts.All(__p => cReadData._isPointInView(__p, axis, __ct)))
        ...
        static pPos _getCPoint(IEnumerable<Mesh3D> meshes)
        {
            var ___elements = meshes.Where(__mesh => __mesh.Verts.Size().Z > 8000)
                .OrderBy(__mesh => __mesh.Verts.Size().X * __mesh.Verts.Size().Y).ToList();

            pPos res = null;

            return ___elements.Count > 0 ? ___elements[0].Verts.CenterPoint() : null;
        }

        public ObjectIdCollection _drawColElevation(string key, PosCollection pls, pPos pt)
        {
            ObjectIdCollection ids = new ObjectIdCollection();

            foreach (var __ls in pls)
            ...
            //pls = pls.Where(__ls => !__ls[0].Content.st_("AXIS")).OrderBy(__ls => -__ls.Area()).ToCollectionSameClosed(true);
            ...
            for (int i = 0; i < pls.Count; i++)
            {
                ...
                if (!__ls[0].Content.st_("AXIS"))
                {
                    ...
                    if (a || b)
                    {
                        ...
                        if (!save_dim_x.Contains(st) && __ls[0].Content != "VAI")
                        {
                            save_dim_x.Add(st);
        ...
        //static double[] levels = new double[] { 0, 6000, 12000, 18000, 25800 };
        ...
        public void DrawMeshViewGroup(string key, pPos _page_pt)
        {
            ...
            //if (i == 0 || i == 3 || i == 4)
            {
                ...
                int lblX = 0, lblY = 0;
                pPos __ct = _getCPoint(this.dicts[key]);

                if (__ct != null)
                {
                    foreach (var mesh in this.dicts[key])
                    {
                        view_pls.AddRange(this.GetFlatView(mesh, axis, __ct));
                        lblX = mesh.LabelX - 1;
                        lblY = mesh.LabelY - 1;
                    }
                    ...
                    if (view_pls.Count > 0)
                    {
                        if (i > 0)
                        {
                            double __nX = 12000 * (i == 1 || i == 2 ? lblX : lblY);
                        ...
                        else
                        {
                            ...
                            double __nY = 12000 * lblY;

                            var __line = new pPos[] { new pPos(__nX + 2000, __nY), new pPos(__nX, __nY), new pPos(__nX, __nY + 2000) };
                            __line[0].Content = __line[1].Content = __line[2].Content = "AXIS_T_X" + (lblX + 1) + "_Y" + (lblY + 1);

                            view_pls.Add(__line);
                        }

                        _drawColElevation(key + " - " + axis.Content, view_pls, _page_pt + new pPos(i == 0 ? -6000 : i * 6000, 0));
            ...
            ACD.DB.Insert("levelBlock", _page_pt + new pPos(6000, 0));
        }

        ObjectIdCollection _draw_grid_line(string lbl_axis, pPos[] __ls)
        {
            ...
            if (ar[1] == "X" || ar[1] == "Y")
            {
                res.Add(ACD.DB.DrawPolyline(__ls, false, GB.HIDDEN_LAYER));
            ...
            else if (ar[1] == "T")
            {
                ...
                res.AddRange(ACD.DB.DrawCircle(__ls[1] + new pPos(-1300, 0), 350));
                res.Add(ACD.DB.CreateText("#M" + ar[2], __ls[1] + new pPos(-1300, 0), 5));

                res.AddRange(ACD.DB.DrawCircle(__ls[1] + new pPos(0, -1000), 350));
                res.Add(ACD.DB.CreateText("#M" + ar[3], __ls[1] + new pPos(0, -1000), 5));
            }
    ...
    public class PolylineElevatorCLS
    {
        ...
        public static void Main(string[] args)
        {
            using (ACD.Lock())
            {
                ...
                MeshList = new Mesh3DCollection(lsTxt, false);
                //Nếu xưởng 5 thì tăng Y lên để bù vào 2 trục cột 4500, nếu không bỏ qua

                


                ACD.ProgressPercent.Location = new System.Drawing.Point(0, 30);

                ACD.ProgressLoad.Maximum = MeshList.dicts.Count;
                ACD.ProgressLoad.Value = 0;
                ACD.ProgressCancel = true;

                string[] current_keys = new string[0];
                //current_keys = new string[] { "X1-Y1" };

                pPos _page_pt = new pPos(-6000, 0);
                int _page_index = 0;

                //ACD.DB.GetEntities(null, EN_SELECT.AC_NAME, GB.TITLE_BLOCK);
                //ObjectId block_title_id = ObjectId.Null;

                //if (IR.SelectedIds.Count > 0)
                //    block_title_id = IR.SelectedIds.First();

                if (current_keys == null || current_keys.Length == 0)

                    foreach (string key in MeshList.dicts.Keys)
                    {
                        ACD.ProgressLoad.Value++;
                    ...
                    foreach (string key in current_keys)
                        if (MeshList.dicts.ContainsKey(key))
                        {
                            MeshList.DrawMeshViewGroup(key, _page_pt);
                            
                            //ACD.DB.MoveObject(block_title_id, _page_pt + title_block_mv - ACD.DB._getPoint(block_title_id));
                ...
                ACD.WR("Thời gian thực hiện: " + elapsedTime);
