...
namespace AcadScript
{
    public class gDraw2DStruct:gDraw2DDCLS
    {
        ...
        public gDraw2DStruct(cReadData css, string _py_filename) : base(css, _py_filename) {}
            ...
            int thep_tren_index, int thep_duoi_index, int slab_cote_type = 0) // 0:Top, 1:Bottom
        {
            ...
            DimObj.category = "b9-dimension";
            DimObj.offset = -F * 2;

            AppendDimension(pt, p2);
            AppendDimension(p2, new pPos(0, -BH) + pt);
            ...
            DimObj.offset = -2.5 * F;
            AppendDimension(new pPos(pt.X, pt.Y), new pPos(pt.X, pt.Y - BH));
            DimObj.offset = -F;
            AppendDimension(new pPos(0, -BH) + pt, new pPos(BW, -BH) + pt);
        }
        ...
        public void DrawBeam(pPos[] src)
        {
            ...
            DimObj.offset = -1.75 * F;
            DimObj.x1 = R[0].X;
            DimObj.x2 = R[1].X;
            AppendDimensionY(R[0].Y); //DIM TỔNG X

            DimObj.offset = F;
            DimObj.y1 = R[0].Y;
            DimObj.y2 = R[2].Y;
            AppendDimensionX(R[0].X); //DIM TỔNG Y
            ...
            for (int i = 0; i < src.Length - 1; i++)
            {
                ...
                if (__R.Size().X > 1500)
                {
                    ...
                    if (__BL >= 1600)
                    {
                        ...
                        DimObj.txt_override = _thep_dai_text(CLP1, thep_dai_index);
                        DimObj.x1 = __R[0].X;
                        DimObj.x2 = thep_tren_trai;
                        AppendDimensionY(__R[2].Y);

                        DimObj.txt_override = _thep_dai_text(CLP2, thep_dai_index);
                        DimObj.x1 = thep_tren_trai;
                        DimObj.x2 = thep_tren_phai;
                        AppendDimensionY(__R[2].Y);

                        DimObj.txt_override = _thep_dai_text(CLP1, thep_dai_index);
                        DimObj.x1 = thep_tren_phai;
                        DimObj.x2 = __R[1].X;
                        AppendDimensionY(__R[2].Y);

                        DimObj.offset = - 2 * F;
                        DimObj.x1 = thep_duoi_trai;
                        DimObj.x2 = __R[0].X;
                        AppendDimensionY( __R[0].Y - 3 * C);

                        DimObj.offset = -2 * F;
                        DimObj.x1 = __R[1].X;
                        DimObj.x2 = thep_duoi_phai;
                        AppendDimensionY(__R[0].Y - 3 * C);
                    }
                ...
                else
                {
                    ...
                    DimObj.txt_override = _thep_dai_text(CLP1, thep_dai_index);
                    AppendDimension(new pPos(__R[0].X, __R[1].Y), new pPos(__R[1].X, __R[1].Y));

                    DimObj.offset = -F * 2;
                    AppendDimension(new pPos(__R[0].X, __R[1].Y), new pPos(__R[1].X, __R[1].Y));

                    DimObj.offset = -F * 1.5;
                    AppendDimension(new pPos(__R[1].X, __R[1].Y), new pPos(__R[0].X, __R[1].Y));
                }
            }
        }
    ...
    public class CadLibStructCLS
    {
        ...
        public static void DrawBeam()
        {
            ...
            if (selIds.Count > 0)
            {
                ...
                region2d = new gDraw2DStruct(CssData, null);
                region2d.DimObj.dimension_architecture_tick = true;
                region2d.DimObj.category = "b9-dimension";
