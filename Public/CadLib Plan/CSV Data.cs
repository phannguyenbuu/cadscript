...
namespace AcadScript
{
    public class CSVDataCLS
    {
        ...
        public static double _getRegionAngle(IEnumerable<pPos> pts)
        {
            pPos[] line = pts.MaxSegment().OrderBy(p => p.X).ThenBy(p => p.Y).ToArray();
            return (line[0] - line[1]).Angle();// / 180 * Math.PI;
        }

        public static pPos _getRegionSize(IEnumerable<pPos> pts, double scale = 1)
        {
        ...
        public List<string> yaw_pitch_clipboard;

        string _format_title(string s)
        {
            s = s.Replace("W_", "");

            if (s.ct_("/")) s = s.filter("/").Last();
            return s;
        }

        public string SaveCSV(cBuilder3D g3d, string filename)
        {
            string alphabet = "ABCDEFGHIJKLMNOQRSTUV";
            yaw_pitch_clipboard = new List<string>();

            foreach (var itm in g3d)
            {
                string mtl = itm.MtlName;
                pPos __txt = cReadData.AreaList.FirstOrDefault(__p => __p.Inside(itm));
                string s_area = "";

                if (__txt != null)
                    s_area = __txt.Content;

                if (!mtl.st_("MT") && alphabet.ct_(mtl[0].ToString()))
                    AppendCSV(itm, mtl, s_area);

                if(!mtl.ct_("ZERO"))
                    yaw_pitch_clipboard.Add("{'text':'" + _format_title(mtl) 
                        + "','title':'" + s_area.Replace(" m2","").ToNumber().ToString("N0") + "'}");
            }

            yaw_pitch_clipboard = yaw_pitch_clipboard.OrderBy(s => s).ToList();
        ...
        public void AppendCSV(IEnumerable<pPos> pts, string s_caption, string s_area, int counter = 0)
        {
